...
- **Màu hỗ trợ (Semantic Colors):**
  - Lỗi (Error/Danger): Đỏ `#EA4335` (Nút Logout, Text báo lỗi)
  - Thành công (Success): Xanh lá `#34A853` (Toast đồng bộ xong, Status hoạt động)
  - Cảnh báo (Warning): Vàng `#FBBC04`
  - Đang chờ (Pending): Xanh lam nhạt `#3498DB` (Trạng thái file đang được tải)
  - Bỏ qua (Skipped): Xám `#95A5A6` (Trạng thái file đã có sẵn, không cần xử lý)
...
- **DataGrid Row (Hàng Bảng Trạng Thái):**
  - Dòng danh sách dạng bảng hiển thị trạng thái file. Nền luân phiên sáng/tối để dễ đọc dọc hàng.
  - Khi có lỗi: Hàng tự nổi đèn (Light Red Highlight) và Auto-Hoist ghim vọt lên top để đập vào mắt người dùng.
...
- **🏠 HomeView (Trang Cấu Hình Chung):** 
  - Công tắc "Khởi động cùng Windows" được bố trí gọn gàng ngay trên Header, ngang hàng với tiêu đề trang.
  - Vùng cài đặt (Settings Container) chia 2 cột song song nằm ngang hàng nhau, bao gồm:
    - Cột 1: Cấu hình Thư mục lưu file `.md` cục bộ (Nút Browse gọi Context Dialog HĐH).
    - Cột 2: Cấu hình chu kỳ Auto-sync (Dropdown OptionMenu và Switch bật/tắt).
...
- **⏱️ SyncView (Lịch Sử Tương Tác Server / File-Centric Dashboard):** 
  - Hiển thị thanh tiến trình (Progress Bar) và Banner đếm tổng các file (Thành công/Lỗi/Bỏ qua).
  - Khung bảng Grid trạng thái thời gian thực. Bám trụ triết lý "Item-Centric" thay vì Log-Centric: Mỗi file là một dòng, tiến trình đi qua bước nào thì dòng đó xuất hiện biểu tượng màu sắc trực quan (⏳/✅/❌/⚪) bước nấy, giúp truy vết lỗi tức thời trên 1 file dễ dàng thay vì cuộn text.
...
### 6.5. Luồng Theo Dõi Lịch Sử Đồng Bộ (File-Centric Dashboard Flow)
```text
[Vào Tab Lịch sử Sync]
       │
[Nhấn Sync Now] ──> [Thanh Progress Bar chạy, reset trắng Bảng danh sách]
       │
       ▼
 [Phát hiện File A từ API] ──> [Sinh ra Row chữ file A trên Bảng]
                                     ├─ Keep Status: ⏳ Đang tải...
                                     ├─ NLM Status:  ⏳ Đợi...
       │
       ▼
 [Worker xử lý xong File A] ──> [Cập nhật Row chữ file A State Map]
                                     ├─ Keep Status: ✅ Đã lưu
                                     ├─ NLM Status:  ⚪ Bỏ qua (đã trùng trên AI)
       │
       ▼
[Nhận được báo lỗi Ghi đè file B] ──> [Row File B lập tức đổi nền chớp đỏ !]
                                     ├─ (Auto-Hoist): Grid ưu tiên bốc Row File B lên đầu danh sách (dòng số 1) để User ấn vào text xem Tooltip mô tả lỗi chi tiết.
```
