...
- **TC-04: Cấp bậc thị giác (Visual Hierarchy):**
  ...
  *   **Keep Client**: Có trách nhiệm đảm bảo lấy đúng cấu trúc JSON/Dict theo interface của thư viện `gkeepapi`.
  *   **Markdown Generator**: Đổi payload linh hoạt về String tùy template cấu hình.
  *   **Output Manager**: Đảm nhiệm tổ chức thư mục trên Disk (Sanitize string, kiểm tra Duplicate name).
  *   **NLM Worker**: Quản lý thao tác CLI tương tác với sổ tay NotebookLM để tạo mới, check hoặc xóa source.
  *   **Logic tối ưu đồng bộ NotebookLM**:
      *   Sử dụng Bulk Source Check để tự động bỏ qua file nếu nó đã tồn tại trên NLM.
      *   Chỉ Trigger NLM Upload khi nội dung file thay đổi; bỏ qua các thay đổi chỉ chứa Metadata.
      *   Xử lý edge-case: Khi file bị mất dưới máy tính, NLM Worker được gọi để xóa bản cũ trên NotebookLM trước khi tải mới lên.
